...
function clearLocalData() {
  ...
  localStorage.removeItem("previcloud-pdf-cartella-custom");
  localStorage.removeItem("previcloud-pdf-salvataggio-modalita");
}
  ...
  return (
    <PageContainer>
      ...
      <div className="mt-4 rounded-2xl border border-edge-faint bg-surface p-6 shadow-sm shadow-brand-navy/[0.03]">
        ...
        <fieldset className="mt-4 border-t border-edge-faint pt-4">
          <legend className="text-sm font-medium text-brand-navy">Come salvare i PDF</legend>
          <div className="mt-3 space-y-2">
            <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-edge px-4 py-3 transition has-[:checked]:border-brand-teal has-[:checked]:bg-brand-teal/5">
              <input
                type="radio"
        ...
        <div className="mt-4 flex items-start justify-between gap-4 border-t border-edge-faint pt-4">
          <div>
        ...
        <div className="mt-5 border-t border-edge-faint pt-4">
          ...
          <p className="mt-3 truncate rounded-lg border border-edge bg-brand-bg px-3 py-2 text-sm text-brand-navy/80">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <button
              ...
              disabled={!desktop}
              className="rounded-xl bg-brand-teal px-5 py-2.5 text-sm font-semibold text-white shadow-sm shadow-brand-teal/25 transition hover:bg-brand-teal/90 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50"
            >
              Scegli cartella
            </button>
            {usaCartellaCustom && (
              <button
                ...
                className="rounded-xl border border-edge px-5 py-2.5 text-sm font-semibold text-brand-navy/70 transition hover:bg-brand-bg"
              >
      ...
      <div className="mt-3 rounded-2xl border border-edge-faint bg-surface p-6 shadow-sm shadow-brand-navy/[0.03]">
        ...
        <button
          ...
          className="mt-4 rounded-xl border border-edge px-5 py-2.5 text-sm font-semibold text-brand-navy transition hover:bg-brand-bg disabled:opacity-50"
        >
        ...
        {updateResult?.update && !updateResult.aggiornato && (
          <button
            ...
            className="mt-3 rounded-xl bg-brand-teal px-5 py-2.5 text-sm font-semibold text-white shadow-sm shadow-brand-teal/25 transition hover:bg-brand-teal/90 active:scale-[0.98] disabled:opacity-50"
          >
            {installingUpdate
        ...
        {updateMsg && <p className="mt-2 text-sm text-brand-teal-ink">{updateMsg}</p>}
      </div>
      ...
      <div className="mt-3 rounded-2xl border border-edge-faint bg-surface p-6 shadow-sm shadow-brand-navy/[0.03]">
        ...
        <div className="mt-3 space-y-2">
          <a
            ...
            className="flex items-center justify-between rounded-xl border border-edge px-4 py-3 text-sm text-brand-navy transition hover:bg-brand-bg"
          >
            <span>Termini di servizio</span>
            <ArrowUpRight size={14} weight="bold" className="text-brand-navy/40" />
          </a>
          <a
      ...
      <div className="mt-3 rounded-2xl border border-edge-faint bg-surface p-6 shadow-sm shadow-brand-navy/[0.03]">
        ...
        <button
          type="button"
          onClick={reset}
          className="mt-3 rounded-xl bg-red-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm shadow-red-600/25 transition hover:bg-red-700 active:scale-[0.98]"
        >
          Cancella dati locali
        </button>
      </div>

      {msg && <p className="mt-3 text-sm text-brand-teal-ink">{msg}</p>}
    </PageContainer>
  );
}
